...
export async function POST(request: NextRequest) {
  try {
    const { content, title, context, customInstruction, baronFeedback, sourceUrl, homepageUrl, category } = await request.json();
...
- 반드시 "~합니다", "~됩니다", "~입니다" 존댓말체로 작성하라. 반말("~다", "~돼", "~해", "~거야") 금지.
- 짧은 문장 위주. 리듬감 유지.
...
수정된 마크다운 본문만 출력하라. 설명이나 메타 코멘트 없이 본문만.`;
      ...
      // 아이디어 포스트 개선 — 원문 분량 유지, 구조 보존
      prompt = `당신은 온라인 수익화 전문 편집자입니다. 아래 블로그 포스트의 문장을 더 매끄럽고 읽기 쉽게 다듬으세요.

[핵심 원칙]
- 원문의 구조(소제목, 문단 순서)를 그대로 유지하라.
- 원문의 분량을 유지하라. 원문 대비 ±20% 이내. 절대 크게 늘리지 마라.
- 새로운 섹션이나 문단을 추가하지 마라.
- 원문의 숫자, 도구명, 플랫폼명은 절대 바꾸지 마라.

[문체 & 톤]
- 비즈니스 분석가가 수익 모델을 설명하는 톤. 담백하고 객관적으로.
- 반드시 "~합니다", "~됩니다", "~입니다" 존댓말체로 작성하라. "~이다", "~것이다", "~점이다", "~된다", "~한다" 같은 문어체/반말 어미 금지. 예: "중요한 점이다" (X) → "중요한 점입니다" (O), "가능하다" (X) → "가능합니다" (O)
- 문장은 짧고 명료하게. 군더더기 없이.
- 핵심 키워드, 도구명, 수치는 볼드(**)로 강조. 문단당 1~2개.

[외부 링크]
- 본문에서 언급하는 도구/플랫폼 중 핵심적인 것 1개만 마크다운 링크로 넣어라.
